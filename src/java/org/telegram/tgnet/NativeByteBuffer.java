...
//    };
    private static final ThreadLocal<NativeByteBuffer> addressWrapper = new ThreadLocal<NativeByteBuffer>() {
        @Override
        protected NativeByteBuffer initialValue() {
            return new NativeByteBuffer(null, true);
        }
    };
//[/sguazt]
...
//    }
    ...
    public static NativeByteBuffer wrap(byte[] bytes)
	{
        NativeByteBuffer result = addressWrapper.get();
        if (bytes != null) {
            if (!result.reused) {
            ...
            result.bytes = bytes;
            result.reused = false;
			result.buffer = ByteBuffer.wrap(bytes);
            result.buffer.order(ByteOrder.LITTLE_ENDIAN);
        }
        return result;
	}
...
//    }
	...
	public NativeByteBuffer(byte[] bytes, boolean wrap)
	{
	}
//[/sguazt]
